...
from util.VITLD_dataset import PL_dataset_WithExpansion
from URMCNet_res_FPFN import TFNet 

def get_sorted_files(directory):
    """
    Fetch and sort valid image files from a specified directory.
    Ensures consistent order across different modalities and labels.
    """
...
def get_single_image_counts_tolerant(pred, target_original, target_expanded, threshold=0.3):
    ...
    # True Positive: Predicted positive AND falls within the expanded region
    tp_mask = pred_mask & gt_exp_mask 
    tp = tp_mask.sum().item()

    # False Positive: Predicted positive BUT falls completely outside the expanded region
    fp_mask = pred_mask & (~gt_exp_mask)
    fp = fp_mask.sum().item()

    # False Negative: Predicted negative BUT belongs to the original core target
    fn_mask = (~pred_mask) & gt_core_mask
    fn = fn_mask.sum().item()

    # True Negative: Ensures total pixel conservation
...
def main(args):
    ...
    ir_test_list = get_sorted_files(args.ir_dir)
    ...
    absolute_ir_test_list = [os.path.join(args.ir_dir, f) for f in ir_test_list]
    ...
    testing_dataset = PL_dataset_WithExpansion(
        absolute_vl_test_list, 
        absolute_ir_test_list, 
